...
4. **禁止通过命令行测试钩子响应**
5. **禁止通过 `cd "path" && echo '{"tool_input": {"command": "..."}' | node script.js` 方式模拟钩子输入进行验证**
...
#### 正常的系统提示：
✅ **"Hook PreToolUse:Bash denied this tool"** - 这是 Claude Code 的标准系统提示

#### 详细的用户提示：
✅ **"⚠️ [命令名]命令被拦截，使用内置的[工具名]代替"** - 这是插件提供的友好中文提示
...
#### 重要说明：
- **"Hook PreToolUse:Bash denied this tool" 是正常的系统提示，不是 bug**
- **用户可以通过切换界面上的 toggle 来查看完整的提示信息**
- **插件会同时返回标准系统提示和友好的中文详细提示**
- **这是预期的正常行为，无需修复**

#### 验证步骤：
1. 执行任何 Bash 命令
2. 查看返回的提示信息
3. 确认有友好的中文提示说明被拦截的原因和替代方案
4. 通过 toggle 可以查看完整的系统信息
...
- **2026-03-30**：更新钩子提示信息说明，明确 "Hook PreToolUse:Bash denied this tool" 是正常系统提示；追加禁止模拟钩子验证的具体规则

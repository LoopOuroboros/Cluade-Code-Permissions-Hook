...
### 2.0.2 (2026-04-24)
- ✨ 新增WSL命令跳过检测：识别 `wsl ...` 命令并整体跳过路径检测，避免引号内Linux glob模式被误判

### 2.0.1 (2026-03-31)
- 🐛 修复WSL命令中Linux路径被误判为Windows路径的问题
- ✨ 改进路径检测算法，增加Linux绝对路径豁免（以/开头的路径）
- ✨ 增强转义序列识别，区分路径反斜杠和转义反斜杠
- ✨ 添加智能命令行参数解析，正确处理带引号的复合参数
- ✅ 确保WSL、Docker等包装命令中的合法Linux路径不被拦截

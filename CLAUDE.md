...
### 🚫 验证规范
- ****禁止模拟测试****: 严禁通过 `cd "path" && echo '{"tool_input": {"command": "..."}}' | node script.js` 方式验证钩子功能
- ****真实环境验证****: 唯一验证方式是在Claude Code真实环境中执行实际命令
- ****禁止测试脚本****: 不得创建或运行任何测试验证脚本
...
| **容器命令** | `docker exec -c "grep..."` | 内部命令拦截 | ✅ 已实现 |
| **远程命令** | `ssh host "cat..."` | 内部命令拦截 | ✅ 已实现 |
